...
from calc_cluster import Hierarchical, AffinityPropagation
from calc_clusterindexstats import sorc_stats, produce_dummy_stats
from demo_calls import calc_pearson_ms, calc_cosine_ms, calc_ssim_ms, calc_multi_feature_similarity, calc_shared_residual_similarity, calc_hypergeometric_similarity, calc_contingency_similarity, calc_mdsi_similarity, calc_local_std_similarity, calc_int_mag_an, calc_gradient_information, calc_intensity_hist, calc_MI
...
parser.add_argument("-pp", "--applypreprocess", type=str2bool, required=True, help="Apply preprocessing?.")
parser.add_argument("-ss", "--applyscalespace", type=str2bool, required=True, help="Apply scale space?.")
parser.add_argument("-sz", "--stepsize", type=int, required=False, choices=[1,2], help="Step size for scale space images.")
parser.add_argument("-pl", "--plotall", type=str2bool, required=False, default=False, help="Plot all images with directories organized according to the cluster structure.")
...
json_mapper = {
    ...
    "Intens Hist": [calc_intensity_hist, False, {}],
    "Mutual Info": [calc_MI, False, {}]
}

stats_dict = {}
for key, value in setup_json[0].items():
    cluster_method = key
    measures = value
    eval_object = {}
    timestamppath = os.path.join(savepath, dataname, pp_ms_parameters, cluster_method)
    ...
    for measure_id in measures:
        ...
        try:
            ...
            if cluster_method == "HierarchicalClustering":
                dmatrix = similarity_distance_switcher(dmatrix)
                cluster = Hierarchical(dmatrix, nr_cluster)
                labels = cluster.perform()
            elif cluster_method == "AffinityPropagationClustering":
                cluster = AffinityPropagation(dmatrix)
                labels = cluster.perform()
                labels = cluster.process_affinity_propagation_memb(labels)
                # Affinity Propagation needs similarities, but Silhouette Score needs distances.
                # Threrefore, the switch is performed after clustering.
                dmatrix = similarity_distance_switcher(dmatrix)
            ...
            eval_object = sorc_stats(dmatrix, imgs, labels, mask_img, measure_id, eval_object)
            print("Finished!")
            print()
            print()
            print()
            ...
            print(e)
            print()
            eval_object = produce_dummy_stats(measure_id, eval_object)        

    f.close()

    stats_dict[cluster_method] = eval_object
